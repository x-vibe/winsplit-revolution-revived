...
#include <commctrl.h>
#include <strsafe.h>  // Security: For StringCchPrintfW

#include <stdio.h>

// Security: Validate HWND before posting messages
inline bool IsValidWinSplitHwnd(HWND hwnd) {
...
LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
  ...
  // Security: Validate HWND before posting any messages
  if (nCode == HC_ACTION && IsValidWinSplitHwnd(hwndWinSplitFrame)) {
...
{
  // Security: Validate HWND before posting any messages
  if (IsValidWinSplitHwnd(hwndWinSplitFrame)) {
    if (nCode == HCBT_SYSCOMMAND && wParam == SC_MOVE) {
...
extern "C" bool DLL_EXPORT InstallAllHook(HWND hwnd)
{
  ...
  WCHAR buffer[256];  // Security: Increased buffer size for safety margin

  if (!InstallMouseHook()) {
    StringCchPrintfW(buffer, ARRAYSIZE(buffer), L"Can not install mouse hook, Error N:%d", static_cast<int>(GetLastError()));
  ...
  if (!InstallCBTHook()) {
    StringCchPrintfW(buffer, ARRAYSIZE(buffer), L"Can not install CBT hook, Error N:%d", static_cast<int>(GetLastError()));
    MessageBox(NULL, buffer, L"Error", MB_OK);

    if (!UninstallMouseHook()) {
      StringCchPrintfW(buffer, ARRAYSIZE(buffer), L"Can not Uninstall mouse hook, Error N:%d", static_cast<int>(GetLastError()));
      MessageBox(NULL, buffer, L"Error", MB_OK);
    }
