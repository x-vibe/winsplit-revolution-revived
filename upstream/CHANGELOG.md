...
- **CRITICAL**: Fixed application startup crash caused by null pointer dereference in XML settings parsing
  - `node->GetAttributes()->GetValue()` crashed when node had no attributes
  - Added null-safety helper function `getNodeValue()` that checks for null before accessing attributes
  - Now supports both XML formats: attribute format (`<Node Value="x"/>`) and text content format (`<Node>x</Node>`)
- Fixed XML node name compatibility for alternate settings file formats
  - Added support for "Numpad"/"VirtualNumpad", "DragAndGo"/"Drag_N_Go" variations
  - Settings files from different WinSplit versions now load correctly
- Fixed language dropdown displaying garbled characters for non-Latin scripts
  - Changed language list to display English names for all 26 supported languages
  - Affected languages: Arabic, Bengali, Chinese (Traditional/Simplified), Greek, Hindi, Japanese, Korean, Persian, Russian, Thai, Ukrainian, Vietnamese

### Added

- File-based debug logging system (`debug_log.h`)
  - Simple singleton logger that writes to `winsplit_debug.log`
  - Thread-safe with mutex protection
  - Timestamped log entries
  - Macros: `DEBUG_LOG(msg)` and `DEBUG_LOG_FMT(fmt, ...)`
  - Disabled by default, enable with `DebugLog::Enable(true)`
- Fluent UI System Icons integration for Windows 11-style appearance
  - Download script to fetch icons from Microsoft's official Fluent UI repo (MIT license)
  - Conversion script to generate PNG, XPM, and ICO formats using ImageMagick
  - Modern replacements for all menu icons: Settings, Keyboard, Help, Info, Power, etc.
  - New tray icon using Fluent "Apps" icon design
  - Multi-resolution application icon (16, 32, 48, 256px)
  - See `images/fluent-icons/README.md` for usage instructions

### Security

- **CRITICAL**: Replaced insecure HTTP update mechanism with secure GitHub Releases API (V1/V10)
  - Uses HTTPS via WinInet to query github.com/x-vibe/winsplit-revolution-revived/releases
  - Eliminates MITM attack vector from plain HTTP update checking
  - New UpdateNotificationDialog shows version comparison, release notes, and GitHub link
  - New NoUpdateDialog for manual check when already on latest version
  - Auto-update checking re-enabled by default with secure implementation
- **CRITICAL**: Added rate limiting to hook message processing (V2)
  - Max 50 messages per 100ms to prevent message flood attacks
- **CRITICAL**: Fixed integer overflow in mouse wheel position tracking (V6)
  - Safe modulo operation that handles negative values correctly
- **HIGH**: Secure DLL unloading with path verification (V3)
  - Verifies DLL loads from application directory before unloading
- **HIGH**: Added SetDefaultDllDirectories for safe DLL search order (V3)
  - Uses LOAD_LIBRARY_SEARCH_APPLICATION_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32
- **HIGH**: Reduced process permissions from PROCESS_ALL_ACCESS to PROCESS_QUERY_LIMITED_INFORMATION (V4)
  - Added graceful fallback to GetWindowText if access denied
- **MEDIUM**: Added bounds checking for all XML integer values (V5)
  - Position values clamped to -10000/10000, percentages to 0-100, timers to 10-1000ms
- **MEDIUM**: Replaced unsafe wsprintf with StringCchPrintfW in hook DLL (V7)
  - Prevents buffer overflow in error message formatting
- **MEDIUM**: Added HWND validation before PostMessage in hook callbacks (V8)
  - Validates window handle exists and is valid before sending messages
- **LOW**: Added privilege elevation detection and warning (V9)
  - Warns users at startup if running with unnecessary administrator privileges
  - Manifest enforces asInvoker (non-elevated) execution level
