    ...
    : wxDialog(parent, id, title)
{
  ...
  p_btnOk->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(ChooseHotkeyDialog::OnButOk), NULL, this);
...
void ChooseHotkeyDialog::CreateControls()
{
  ...
  p_cmbMod1 = new wxComboBox(
      this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(55, 20), 0, NULL, wxCB_READONLY);
  p_cmbMod2 = new wxComboBox(
      this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(55, 20), 0, NULL, wxCB_READONLY);
  p_cmbVirtKey = new wxComboBox(
      this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(100, 20), 0, NULL, wxCB_READONLY);
...
HotkeyStruct ChooseHotkeyDialog::GetHotkeys()
{
  ...
  st_hk.modifier2 = m_mod_manager.GetValueFromIndex(
      p_cmbMod2->GetSelection()); // tab_modifier[p_cmbMod2->GetSelection()];
...
{
  p_cmbMod1->SetSelection(m_mod_manager.GetIndexFromValue(hk.modifier1) -
                          1); // GetIndexVal(tab_modifier,5,hk.modifier1)-1);
  ...
  for (unsigned int i = 0; i < vec_hk.size(); ++i) {
    if (i != (unsigned int)n_item)
      if (vec_hk[i].modifier1 == m_mod_manager.GetValueFromIndex(p_cmbMod1->GetSelection() + 1) &&
        ...
        wxMessageBox(
            _("This hotkey is already used by WinSplit"), _("WinSplit message"), wxOK, this);
        return;
      }
  }

  if (!(::RegisterHotKey((HWND)GetHandle(),
                         id,
                         m_mod_manager.GetValueFromIndex(p_cmbMod1->GetSelection() + 1) |
                             m_mod_manager.GetValueFromIndex(p_cmbMod2->GetSelection()),
...
void HotkeyConfigureDialog::CreateControls()
{
  ...
  pmainSizer->Add(new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxSize(10, -1)),
                  1,
...
{
  p_btnActiv->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                      wxCommandEventHandler(HotkeyConfigureDialog::OnButActiv),
                      NULL,
                      this);
  p_btnEdit->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                     wxCommandEventHandler(HotkeyConfigureDialog::OnButEdit),
                     NULL,
                     this);
  p_listbox->Connect(wxEVT_COMMAND_LIST_ITEM_SELECTED,
                     wxListEventHandler(HotkeyConfigureDialog::OnTaskChangeSelection),
                     NULL,
                     this);
  p_listbox->Connect(wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
                     wxListEventHandler(HotkeyConfigureDialog::OnTaskDoubleClick),
                     NULL,
                     this);
}
...
void HotkeyConfigureDialog::OnButActiv(wxCommandEvent& event)
{
  ...
  if (vec_tab_tmp[m_item_selected].session) {
    ...
    val = ::RegisterHotKey((HWND)GetHandle(),
                           0,
                           vec_tab_tmp[m_item_selected].modifier1 |
                               vec_tab_tmp[m_item_selected].modifier2,
