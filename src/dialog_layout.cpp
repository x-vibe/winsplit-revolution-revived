    ...
    , p_btnCancel(NULL)
{
  ...
  p_btnOk->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                   wxCommandEventHandler(ChooseRatioDialog::CheckValidity),
                   NULL,
                   this);
}
...
void ChooseRatioDialog::CreateControls()
{
  ...
  p_txtCtrlX = new wxTextCtrl(this,
                              wxID_ANY,
                              _T ("0.00"),
                              wxDefaultPosition,
                              wxSize(40, -1),
  ...
  p_txtCtrlY = new wxTextCtrl(this,
                              wxID_ANY,
                              _T ("0.00"),
                              wxDefaultPosition,
                              wxSize(40, -1),
                              0,
                              wxTextValidator(wxFILTER_NUMERIC));
  p_txtCtrlWidth = new wxTextCtrl(this,
                                  wxID_ANY,
                                  _T ("0.00"),
                                  wxDefaultPosition,
                                  wxSize(40, -1),
                                  0,
                                  wxTextValidator(wxFILTER_NUMERIC));
  p_txtCtrlHeight = new wxTextCtrl(this,
                                   wxID_ANY,
                                   _T ("0.00"),
                                   wxDefaultPosition,
                                   wxSize(40, -1),
  ...
  main_sizer->Add(new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxSize(10, 1)),
                  1,
...
void LayoutDialog::CreateControls()
{
  ...
  p_lstCtrlTask =
      new wxListCtrl(this,
                     ...
                     wxDefaultPosition,
                     wxSize(130, 150),
                     wxLC_REPORT | wxSTATIC_BORDER | wxLC_SINGLE_SEL | wxLC_VRULES | wxLC_HRULES);
  p_lstCtrlCombo =
      new wxListCtrl(this,
                     wxID_ANY,
                     wxDefaultPosition,
  ...
  hsizer_panel->Add(
      new wxStaticBitmap(this, wxID_ANY, wxBitmap(im_schema)), 0, wxALIGN_CENTRE | wxALL, 5);
...
{
  p_lstCtrlTask->Connect(wxEVT_COMMAND_LIST_ITEM_SELECTED,
                         wxListEventHandler(LayoutDialog::OnTaskChangeSelection),
                         NULL,
                         this);
  p_lstCtrlCombo->Connect(wxEVT_COMMAND_LIST_ITEM_SELECTED,
                          wxListEventHandler(LayoutDialog::OnComboChangeSelection),
                          NULL,
                          this);
  p_lstCtrlCombo->Connect(wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
                          wxListEventHandler(LayoutDialog::OnActiveCombo),
                          NULL,
                          this);
  p_btnAdd->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(LayoutDialog::OnAddCombo), NULL, this);
  p_btnEdit->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(LayoutDialog::OnEditCombo), NULL, this);
  p_btnDelete->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(LayoutDialog::OnDeleteCombo), NULL, this);
  p_spinBtnUpDown->Connect(
      wxEVT_SCROLL_LINEDOWN, wxSpinEventHandler(LayoutDialog::OnSpinDown), NULL, this);
  p_spinBtnUpDown->Connect(
      wxEVT_SCROLL_LINEUP, wxSpinEventHandler(LayoutDialog::OnSpinUp), NULL, this);
  p_btnOk->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(LayoutDialog::OnOKBtn), NULL, this);
}
...
void LayoutDialog::OnComboChangeSelection(wxListEvent& event)
{
  ...
  value << _T ("x: ")
        << wxString::Format(_T ("%.2f"), tab_seq_tmp[m_item_task_selected][m_item_combo_selected].x)
        << _T ("%");
  p_stc_x->SetLabel(value);

  value.Clear();
  value << _T ("y: ")
        << wxString::Format(_T ("%.2f"), tab_seq_tmp[m_item_task_selected][m_item_combo_selected].y)
        << _T ("%");
  ...
  value << _("width") << _T (": ")
        << wxString::Format(_T ("%.2f"),
                            tab_seq_tmp[m_item_task_selected][m_item_combo_selected].width)
        << _T ("%");
  ...
  value << _("height") << _T (": ")
        << wxString::Format(_T ("%.2f"),
                            tab_seq_tmp[m_item_task_selected][m_item_combo_selected].height)
        << _T ("%");
...
void LayoutDialog::RefreshLstCtrlCombo(bool select_first)
{
  ...
  if (select_first) {
    ...
    p_lstCtrlCombo->SetItemState(
        m_item_combo_selected, wxLIST_STATE_SELECTED, wxLIST_STATE_SELECTED);
  }
}
...
void LayoutDialog::OnDeleteCombo(wxCommandEvent& event)
{
  ...
  if (wxMessageBox(_("Are you sure you want delete this entry?"), _("Warning"), wxYES | wxNO) ==
      wxYES) {
    tab_seq_tmp[m_item_task_selected].erase(tab_seq_tmp[m_item_task_selected].begin() +
...
void LayoutDialog::OnSpinUp(wxSpinEvent& event)
{
  ...
  if (tab_seq_tmp[m_item_task_selected].size() > 1) {
    swap(tab_seq_tmp[m_item_task_selected][m_item_combo_selected],
         tab_seq_tmp[m_item_task_selected][m_item_combo_selected - 1]);
