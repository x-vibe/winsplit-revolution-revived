...
DownloadThread::DownloadThread(DownloadDialog* parent, const wxString& urlHost,
                               const wxString& fileLocal)
...
void DownloadDialog::CreateControls()
{
  ...
  p_sctTextDownload = new wxStaticText(
  ...
  vsizer->Add(new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxSize(150, -1)),
              0,
              wxEXPAND | wxALL | wxALIGN_CENTER,
              10);
...
void DownloadDialog::IniText(const wxString& versionFileInfo)
{
  ...
  message << _("A new version of WinSplit Revolution is available:") << _T (" ") << version
          << _T ("\n\n");
  message << _("Press next to download the last version.") << _T ("\n");
  message << _(
      "You can visit the website https://github.com/dozius/winsplit-revolution to download it.");
...
{
  p_btnNext->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(DownloadDialog::OnNext), NULL, this);
  p_btnCancel->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(DownloadDialog::OnCancel), NULL, this);
...
void DownloadDialog::OnCancel(wxCommandEvent& event)
{
  if (p_thread) {
    if (p_thread->IsRunning()) {
      p_thread->Pause();
      if (wxMessageBox(_("Are you sure you want to stop downloading the new version?"),
                       _("Warning"),
                       wxYES_NO,
                       this) == wxYES) {
        p_thread->Delete();
